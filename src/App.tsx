...
>
      ...
      {type === \'invoice\' && <section className="doc-totals"><div className="totals-rows"><div><span>Subtotal</span><b>{money(subtotal)}</b></div><div><span>Discount</span><b>{money(Number(doc.discount || 0))}</b></div><div><span>Tax ({settings?.default_tax || 0}%)</span><b>{money(tax)}</b></div><div className="grand-row"><span>Grand total</span><strong>{money(grand)}</strong></div>{type === 'invoice' && <><div><span>Amount paid</span><b>{money(Number(doc.amount_paid || 0))}</b></div><div className="balance-row"><span>Balance due</span><strong>{money(balance)}</strong></div></>}</div></section>}
